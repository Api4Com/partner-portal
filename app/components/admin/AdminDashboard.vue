    ...
    <div class="mx-auto max-w-[1240px] space-y-6">
        ...
        <div v-for="h in HORIZONS" :key="h.id" class="overflow-hidden rounded-2xl border border-default bg-default">
          ...
          <ul class="divide-y divide-default">
            <li v-for="item in columnItems(h.id)" :key="item.id" class="flex items-center gap-3 px-4 py-3">
              ...
              >
                <p class="truncate text-sm font-medium hover:text-primary">{{ item.title }}</p>
              ...
              >
                <span class="inline-flex items-center gap-1 text-emerald-600">
                  <UIcon name="i-lucide-thumbs-up" class="h-3.5 w-3.5" /> {{ data.states[item.id]?.likeCount ?? 0 }}
                </span>
                <span class="inline-flex items-center gap-1 text-rose-600">
                  <UIcon name="i-lucide-thumbs-down" class="h-3.5 w-3.5" /> {{ data.states[item.id]?.dislikeCount ?? 0 }}
                </span>
                <span class="inline-flex items-center gap-1" :class="commentCountOf(item.id) ? 'text-primary' : ''">
                  <UIcon name="i-lucide-message-square" class="h-3.5 w-3.5" /> {{ commentCountOf(item.id) }}
                </span>
              </button>
              <UBadge v-if="!item.published" color="neutral" variant="subtle" size="sm">não visível</UBadge>
              <UButton
      ...
      <div v-else class="overflow-hidden rounded-2xl border border-default bg-default">
        ...
        <ul v-else class="divide-y divide-default">
          <li v-for="entry in feed" :key="entry.kind + entry.id" class="flex items-start gap-3 px-4 py-3">
            <span
            ...
            <div class="min-w-0 flex-1">
              <p class="text-sm">
                ...
                <span v-if="entry.company" class="text-dimmed"> · {{ entry.company }}</span>
                <span class="text-muted">
                  {{ entry.kind === 'comment' ? ' comentou em ' : entry.reaction === 'like' ? ' curtiu ' : ' não curtiu ' }}
                </span>
                <span class="font-medium">{{ entry.itemTitle }}</span>
              </p>
              <p v-if="entry.kind === 'comment'" class="mt-1 whitespace-pre-wrap text-sm leading-relaxed text-muted">
                {{ entry.body }}
              </p>
            </div>
