...
<template>
  ...
  >
    <div class="flex items-start justify-between gap-2">
      <span class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-semibold" :class="tag.className">
        <UIcon v-if="item.tag === 'breaking-change'" name="i-lucide-triangle-alert" class="h-3 w-3" />
        {{ tag.label }}
      </span>
    ...
    <div class="mt-3 flex items-center justify-between border-t border-default pt-3">
      ...
      <span v-else class="inline-flex items-center gap-1.5 text-xs font-medium text-muted">
      ...
      <template v-if="profile === 'commercial'">
        <span v-if="interested" class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600">
          <UIcon name="i-lucide-circle-check" class="h-3.5 w-3.5" /> Lead marcado
        </span>
        <span v-else class="inline-flex items-center gap-1 text-xs font-medium text-primary">
          <UIcon name="i-lucide-trending-up" class="h-3.5 w-3.5" /> Ver valor
        </span>
      </template>
      <template v-else>
        <span v-if="betaRequested" class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600">
          <UIcon name="i-lucide-circle-check" class="h-3.5 w-3.5" /> Beta solicitado
        </span>
        <span v-else class="text-xs font-medium text-primary">Ver detalhes</span>
      </template>
    </div>
  </button>
</template>
