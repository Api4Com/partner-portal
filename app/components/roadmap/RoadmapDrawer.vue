...
<template>
  <USlideover v-model:open="open" :title="activeItem?.title" :description="activeItem?.summary">
    <template #body>
      <div v-if="activeItem" class="space-y-5">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <span v-if="tag" class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-semibold" :class="tag.className">
            {{ tag.label }}
          </span>
          <span v-if="horizon" class="rounded-md bg-muted px-2 py-0.5 text-[11px] font-medium text-muted ring-1 ring-inset ring-default">
            {{ horizon.title }}
          </span>
        </div>
        ...
        <div v-if="tab === 'commercial'" class="space-y-6">
          <section>
            ...
            <p class="mt-3 text-sm leading-relaxed text-muted">{{ activeItem.commercial.businessValue }}</p>
          </section>
              ...
              >
                <span class="flex items-center gap-3">
                  <span class="grid h-8 w-8 place-items-center rounded-lg bg-brand-50 text-brand-600">
                    <UIcon :name="asset.type === 'pricing' ? 'i-lucide-tag' : 'i-lucide-file-text'" class="h-4 w-4" />
                  </span>
                  <span>
        ...
        <div v-else class="space-y-6">
          ...
          <section>
            <div class="mb-2.5 flex items-center gap-2">
              <UIcon name="i-lucide-code-2" class="h-4 w-4 text-primary" />
              <h4 class="text-xs font-semibold uppercase tracking-wider text-muted">Endpoints Envolvidos</h4>
            </div>
            <div class="space-y-2">
              <div
              ...
              >
                <span class="inline-flex w-14 justify-center rounded-md px-1.5 py-0.5 text-[10px] font-bold tracking-wide ring-1 ring-inset" :class="METHOD_COLORS[ep.method]">
                  {{ ep.method }}
                </span>
          ...
          <section v-if="activeItem.technical.webhooks && activeItem.technical.webhooks.length">
            <div class="mb-2.5 flex items-center gap-2">
              <UIcon name="i-lucide-webhook" class="h-4 w-4 text-primary" />
              <h4 class="text-xs font-semibold uppercase tracking-wider text-muted">Eventos de Webhook</h4>
            </div>
          ...
          <section v-if="activeItem.technical.samplePayload">
            <div class="mb-2.5 flex items-center gap-2">
              ...
              <h4 class="text-xs font-semibold uppercase tracking-wider text-muted">Exemplo de Payload</h4>
            </div>
    ...
    <template #footer>
      <div v-if="activeItem" class="w-full">
        <UButton
          v-if="tab === 'commercial'"
          block
          size="lg"
